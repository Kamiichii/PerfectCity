...
def use_ai(user_input,system_prompt):
    ...
    response = _client.models.generate_content(model="gemini-2.0-flash-001", contents=_messages,config=types.GenerateContentConfig(system_instruction=system_prompt))
    _messages += [
    types.Content(role="model", parts=[types.Part(text=response.text)]),
    ]
    _console.print(Panel(response.text,title="City Description",border_style="grey46"))
